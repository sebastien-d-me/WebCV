...
const projectsButtons: NodeListOf <HTMLButtonElement> = document.querySelectorAll(".button--project");
const projectsError: HTMLElement | null = document.querySelector(".projects__error");
const projectsList: NodeListOf <HTMLElement> = document.querySelectorAll(".projects__list");
const projectsSites: HTMLElement | null = document.querySelector(".projects__sites");
const projectsModules: HTMLElement | null = document.querySelector(".projects__modules");
const projectsTools: HTMLElement | null = document.querySelector(".projects__tools");


// Interface declaration for projects
interface Project {
    name: string;
    created_at: string;
    homepage: string;
...
fetch("https://api.github.com/users/sebastien-d-me/repos")
    ...
    .then((response) => {
        ...
        if(response !== "Error") {
            ...
            const allProjects: Project[] = response.sort(() => 0.5 - Math.random());
...
// Function to display the projects of the selected category
type Category = "sites" | "modules" | "tools";

const manageProjects = (category: Category) => {
...
projectsButtons.forEach(button => {
    button.addEventListener("click", (event: Event) => {
        const category = (event.target as HTMLButtonElement).getAttribute("data-category") as Category;
